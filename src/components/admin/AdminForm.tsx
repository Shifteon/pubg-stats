...
import { useTeamOverview, useTeamGames, useTeamGame, useTeamsList } from "@/hooks/useTeam";
import {
  Form,
  Input,
  Button,
...
export default function AdminForm({ teamId, isOpen, onOpenChange, isEdit, initialGame }: AdminFormProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      } else if (initialGame?.teamId) {
        setActiveTeamId(initialGame.teamId);
      }
    }
  }, [teamId, isOpen, isEdit, initialGame]);

  const { teamsList, isLoading: isTeamsLoading } = useTeamsList();
  const { teamOverview, isLoading, isError } = useTeamOverview(activeTeamId);
  const { teamGames, mutate: mutateGames } = useTeamGames(activeTeamId);
  ...
  const { game, mutate: mutateGame } = useTeamGame(activeTeamId, gameId);
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const endpoint = isEdit && initialGame
      ...
      : `/api/team/${activeTeamId}/new/game`;
            ...
            <ModalBody>
              ...
              {teamsList &&
                <Select
                  ...
                  isDisabled={isEdit} // Do not allow changing team if editing
